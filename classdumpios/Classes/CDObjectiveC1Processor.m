...
- (void)processModules;
{
    ...
    while ([cursor isAtEnd] == NO) {
        ...
        DLog(@"%08x %08x %08x %08x - '%@'", objcModule.version, objcModule.size, objcModule.name, objcModule.symtab, name);
        //DLog(@"\tsect: %@", [[machOFile segmentContainingAddress:objcModule.name] sectionContainingAddress:objcModule.name]);
        //DLog(@"symtab: %08x", objcModule.symtab);
...
- (CDOCSymtab *)processSymtabAtAddress:(uint32_t)address;
{
    ...
    //DLog(@"[@ %08x]: %08x %08x %04x %04x", address, objcSymtab.sel_ref_cnt, objcSymtab.refs, objcSymtab.cls_def_count, objcSymtab.cat_def_count);
...
- (CDOCClass *)processClassDefinitionAtAddress:(uint32_t)address;
{
    ...
    // Process meta class
    {
        ...
        //DLog(@"meta class, isa = %08x", objcClass.isa);
        ...
        DLog(@"metaclass= isa:%08x super:%08x  name:%08x ver:%08x  info:%08x isize:%08x  ivar:%08x meth:%08x  cache:%08x proto:%08x",
...
- (CDOCProtocol *)protocolAtAddress:(uint32_t)address;
{
    ...
    if (protocol == nil) {
        //DLog(@"Creating new protocol from address: 0x%08x", address);
        ...
        //DLog(@"data offset for %08x: %08x", v2, [machOFile dataOffsetForAddress:v2]);
        //DLog(@"[@ %08x] v1-5: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x (%@)", address, v1, v2, v3, v4, v5, name);

        {
            ...
            if (v3 != 0) {
                ...
                //DLog(@"val: 0x%08x", val);
                uint32_t count = [cursor readInt32];
                //DLog(@"protocol count: %08x", count);
                for (uint32_t index = 0; index < count; index++) {
                    val = [cursor readInt32];
                    //DLog(@"val[%2d]: 0x%08x", index, val);
        ...
        //DLog(@"Found existing protocol at address: 0x%08x", address);
    }
