...
- (void)processModules;
{
    ...
    while ([cursor isAtEnd] == NO) {
        ...
        DBLog(@"objcModule.size: %u", objcModule.size);
        DBLog(@"sizeof(struct cd_objc_module): %lu", sizeof(struct cd_objc_module));
        ...
        DBLog(@"%08x %08x %08x %08x - '%@'", objcModule.version, objcModule.size, objcModule.name, objcModule.symtab, name);
        //DBLog(@"\tsect: %@", [[machOFile segmentContainingAddress:objcModule.name] sectionContainingAddress:objcModule.name]);
        //DBLog(@"symtab: %08x", objcModule.symtab);
...
- (CDOCSymtab *)processSymtabAtAddress:(uint32_t)address;
{
    ...
    //DBLog(@"[@ %08x]: %08x %08x %04x %04x", address, objcSymtab.sel_ref_cnt, objcSymtab.refs, objcSymtab.cls_def_count, objcSymtab.cat_def_count);
...
- (CDOCClass *)processClassDefinitionAtAddress:(uint32_t)address;
{
    ...
    //DBLog(@"name: %08x", objcClass.name);
    //DBLog(@"className = %@", className);
    if (className == nil) {
    ...
    // Process meta class
    {
        ...
        //DBLog(@"meta class, isa = %08x", objcClass.isa);
        ...
        DBLog(@"metaclass= isa:%08x super:%08x  name:%08x ver:%08x  info:%08x isize:%08x  ivar:%08x meth:%08x  cache:%08x proto:%08x",
...
- (CDOCProtocol *)protocolAtAddress:(uint32_t)address;
{
    ...
    if (protocol == nil) {
        //DBLog(@"Creating new protocol from address: 0x%08x", address);
        ...
        //DBLog(@"data offset for %08x: %08x", v2, [machOFile dataOffsetForAddress:v2]);
        //DBLog(@"[@ %08x] v1-5: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x (%@)", address, v1, v2, v3, v4, v5, name);

        {
            ...
            if (v3 != 0) {
                ...
                //DBLog(@"val: 0x%08x", val);
                uint32_t count = [cursor readInt32];
                //DBLog(@"protocol count: %08x", count);
                for (uint32_t index = 0; index < count; index++) {
                    val = [cursor readInt32];
                    //DBLog(@"val[%2d]: 0x%08x", index, val);
        ...
        //DBLog(@"Found existing protocol at address: 0x%08x", address);
    }
